...
* `-i` _file_ - initial source frame.  (default to 'blank.html')  See also 'initialsource' semlit command.
...
The initialsource command identifies a source file to be initially displayed
in the source frame.
It defaults to "blank.html".
This should be the final source output file, as generated by semlit.
For example, "xyz_c.slsrc.html"
This command is often included right after the corresponding 'srcfile'
command (see above).

No output text is generated by this command.
It only sets internal state of the semlit tool so that the right file is
specified in the frameset page.

Form:

    =semlit,initialsource,file=

Example:

    =semlit,initialsource,semlit_sh.slsrc.html=
